```
Emblem Associated with AnonMafia
Cyber Family 2014-present day
```
```
Emblem Associated with AnonMafia
2014-
```
# AnonMafia Cyber Family

## AnonMafia Cyber Family (hacker group)

**AnonMafia Cyber Family** , also known as **AMCF** or
**AnonMafia** , is an hacktivist collective operating as an
offshoot of the Anonymous hacking collective. The group
formed on Facebook in September 2014 (originally as
**AnonMafia** ) during the 2014 Hong Kong protests, they were
outraged by the treatment thus response from the Hong Kong
government and local authorities towards the protesting
students, this led them to their first cyber operation known as
#OpHongKong, which they later contacted an independent
journalist known as Ford Fischer (Founder of News2Share) to
help spread notoriety of the operation, this minor
collaboration led to the first video ever, of Anonymous
addressing the protest in Hong Kong (The video no longer
exist nor can be found).

On October 3 2014, they decided to move their cyber
operations to twitter in hopes to reach a broader audience,
leading to a change of their name from **AnonMafia** to
**AnonMafia Cyber Family** and also merged with another
group called **Ghost Security Anonymous (GSA)** who
became adopted under the **AMCF** umbrella, they eventually
tied themselves into a new up and coming operation known as
#OpKKK, they later re-named #OpTakkk for themselves.
During the operation, they launched a multitude amount of
Denial-of-service attack against numerous KKK-linked
websites, including a website belonging to the **Traditionalist American Knights of the Ku Klux Klan
(TAK),** a white supremacist hate group, largely based in Missouri and active during the 2010s, which was
founded and led by Imperial Wizard _Frank Ancona_ (later murdered in 2017). Who they later doxed and
released his information to the public. Their involvement with this operation put **AnonMafia Cyber
Family** on the map in the Anonymous (hacker group) community.

```
Operation Hong Kong 2014 (#OpHongKong) - This was the first operation AnonMafia
participated in.
Operation Klu Klux Klan 2014 (#OpKKK or #OpTakkk)
Operation Ferguson 2014 (#OpFerguson)
```
## Cyber Operations (2014-2016)


```
Operation China 2014 (#OpChina)
Operation PirateBay 2014 (#OpPirateBay)
Operation Israel 2014 (#OpIsrael or #OpIsrael2014)
Operation Stop YuLin 2015 (#OpStopYuLin)
Operation North Korea 2015 (#OpNorthKorea)
Operation Kendrick Johnson 2015 (#OpKendrickJohnson)
Operation Israel 2015 (#OpIsrael or #OpIsrael2015)
Operation ISIS 2015 (#OpISIS or #OpIceISIS)
Operation Charlie Hebdo 2015 (#OpCharlieHebdo)
Operation Paris 2015 (#OpParis)
Operation Bay of Pigs 2015 (#OpBayofPigs)
Operation BaltimorePD 2015 (#OpBaltimorePD)
Operation BEAST 2015-2016 (#OpBEAST) - This was the last operation AnonMafia Cyber
Family participated in.
```
**AnonMafia Cyber Family** released a large amount of doxing information on local law enforcement in
Missouri, New York City, Ferguson, and Kansas City during Operation Ferguson, Operation Kendrick
Johnson, and Operation BaltimorePD to the public. As well as launching crucial Denial-of-service attacks
against government websites as protest against treatment of the Kendrick Johnson case and Freddie Grey.

- #OpFerguson #OpKendrickJohnson #OpBaltimorePD #OpBayOfPigs

**AnonMafia Cyber Family** targeted the swedish government as payback for Swedish law enforcement
seizing Pirate Bay servers resulting in the permanent closure of the Pirate Bay platform - #OpPirateBay

**AnonMafia Cyber Family** hacked into and defaced ISIS-linked websites, as well as launched numerous
Denial-of-service attacks to ISIS-linked Websites and services, and accused cloudflare on multiple
occasions of protecting ISIS-linked websites hosted on their platform. Forcing ISIS-linked website users
to retreat to Telegram for communication. - #OpCharlieHebdo #OpISIS #OpParis

**AnonMafia Cyber Family** launched numerous Denial-of-service attacks against websites that contained
Bestiality content, as well as reporting the sites to their hosting providers, resulting in the termination of
hundreds of sites, also accusing a chicago-based company FDC Servers (https://fdcservers.net/) of
allowing Bestiality websites to be hosted on their platform. - #OpBEAST

**AnonMafia Cyber Family** hacked into and defaced Chinese government websites, as well as launched
numerous Denial-of-service attacks to Chinese government websites and services, to protest the Yulin
Dog Meat Festival. - #OpStopYuLin #OpChina

**AnonMafia Cyber Family** launched numerous Denial-of-service attacks against Israeli government
websites and services, alongside AnonGhost and Middle East Cyber Army (MECA) to protest against
Zionism and unruly treatment of Palestinians. - #OpIsrael

```
(Founder) M4STER T3MPER aka SmurfTroll *Retired after DerpLaughing and
IncursioSubter got arrested
(Co-Founder) DerpLaughing *Was arrested for hacking CIA Director John Brennan
```
## Members (2014-2016 / list confirmed via X posts)


```
email in 2016, he was also in a group called Crackas Wit Attitudes
IncursioSubter *Was arrested along with DerpLaughing for hacking CIA Director John
Brennan email in 2016, he also was in the group Crackas Wit Attitudes
AnonRabb *Retired
GSA Romeo *Retired
AnonTerminal *Retired
Deployed_Drone *Retired
BLU3ST4R aka BL4CK_FL4G *Retired
SilentX Philippines *Retired
H17@k *Retired
GSA Loud *Retired
Oh Hax *Retired
PRO 666 *Retired
R3p3aR *Retired
JibsWiz *Retired
D34D1Y^GUMMY^B3AR *Retired
VenuM aka VenOmz * Retired
Scream4.0.4 *Retired
Mr.xSaputra^AttackRx *Retired
1qb41&1vy ch3n *Retired
koe mala *Retired
TheKidz404 *Retired
EML^404 *Retired
Cysix *Retired
RedHaxx *Retired
cyber^polic3^007 *Retired
Shafrul^Anon *Retired
etqeteq *Retired
b3t4.xx. irvin *Retired
Anons DevilNia *Retired
Nick TAC007 *Retired
AMCF IAB *Retired
```
It was confirmed that while in teenage, M4STER T3MPER and DerpLaughing met on the Xbox Live
platform, from there, they then form **AnonMafia** together.

After the arrest of DerpLaughing and IncursioSubter for the hacking of CIA Director John Brennan AOL
email account in 2016, members of **AnonMafia Cyber Family** all decided to disband and end their
hacktivist career. In fact, till this very day, many of them have chosen to stay in very close contact and be
completely "Anonymous".

Articles, Software(s), and Social Media accounts (X formerly twitter) still exist to this very day, leaving a
footprint of **AnonMafia Cyber Family's** existence on the internet, leaving a reputation as pioneer's and
legend's who have paved the way for many hacking groups, leaving behind a gigantic trail of extreme
cyber warfare, launching massive Denial-of-service attacks, Hacking into various kinds of servers,
Building botnets, Building web stressors, Doxing rivals and releasing their information to the public, and


Coding ferocious hacking tools thus bringing complete cyber devastation to their enemies. Targeting big
governments, other hacking groups (They had quarrels with Lizard Squad and BinarySec (https://x.com/R
ealBinarySec) at one point), and cyber terrorists organizations.

#AnonMafiaCyberFamily

#AMCF

#Anonymous

AnonMafia Cyber Family DOSER (Modified version of Low Orbit Ion Cannon, resembles AnonGhost
DDOS) - This tool was created and heavily used for #OpIsrael2015 #OpKKK #OpChina #OpPirateBay

B4CKSELF V6 (https://www.inforge.net/forum/threads/b4ckself-v6-download-open-source-cloudflare-b
ypass-proxifier3.517639/) (a Python 3 , Layer 7 DDoS Tool that uses proxies for anonymity and
Cloudflare scripts to bypass Cloudlflare DDoS Protection) - This tool was heavily used for #OpBEAST
#OpISIS #OpPirateBay

AnonMafia Web Cannon (A Javascript/HTML and modified version of Low Orbit Web Cannon and
AnonMafia Cyber Family would actually host this script as a website on Google Drive in 2014-
using Google Servers to execute DDoS attacks) - This tool was heavily used for #OpFerguson
#OpBaltimorePD #OpKendrickJohnson #OpKKK #OpChina #OpHongKong

THC-SSL-DOS (https://www.kali.org/tools/thc-ssl-dos/) (A open source DDoS tool coded to specifically
target HTTPS/SSL servers and websites) - This tool was heavily used for #OpPirateBay

Mobhammer (A modified version of Torshammer) - This tool was used for every cyber operation

Slowloris (A Perl script DDoS Tool) - This tool was used for every cyber operation

R U Dead Yet aka R.U.D.Y (A open source, python Layer 7 DDoS Tool) - This tool was used for every
cyber operation

High Orbit Ion Cannon (A open source, DDoS tool written in VB and C#) - This tool was heavily used
for #OpKKK #OpHongKong

Pyloris (a Python DDoS tool that uses Tor and other proxies to for anonymity) - This tool was used for
every cyber operation

GoldenEye (https://www.kali.org/tools/goldeneye/) (A Python 3 , Layer 7 DDoS Tool) - This tool was
used for every cyber operation

Hulk (https://github.com/HkerVit/Hulk-ddos-attack) (a Python, Layer 7 DDoS Tool) - This tool was used
for every cyber operation

### X.com handles (formerly twitter)

## DDoS Tools (Used by AnonMafia Cyber Family)


XerXes (https://github.com/Dev0uss/xerxes/blob/master/xerxes.c) (a C, Layer 4 DDoS Tool that uses
Tor) - This tool was used for every cyber operation

XMLRPC DDoS (https://www.wallarm.com/what/what-is-xml-rpc-and-why-do-you-need-to-secure-it) (a
Layer 7 DDoS Tool that uses vulnerable WordPress websites as bots) - This tool was heavily used in
#OpHongKong and #OpChina, many articles discuss this

anonmafiastresser.com (a HTML/Javascript/PHP website that provided DDoS-for-hire services to the
public) - This website was owned and operated by **AnonMafia Cyber Family** for a short period to raise
money, the FBI and other Law Enforcement Agencies formed Operation PowerOFF to eliminate all
Streesser/Booter websites online.

SQLmap (https://sqlmap.org/)(a SQL injection tool)

Nmap (https://nmap.org/) (Network scanning, port discovery, OS fingerprinting)

Hping (https://github.com/antirez/hping) (Packet crafting, firewall testing)

Whois (https://who.is/) (Domain + DNS reconnaissance)

NSlookup (https://www.nslookup.io/) (Domain + DNS reconnaissance)

Backtrack 5 (https://www.backtrack-linux.org/) (pre-Kali Linux)

Kali Linux (https://www.kali.org/) (Successor of Backtrack 5)

Metasploit Framework (https://www.metasploit.com/) (Modular exploit development + testing)

Nikto (https://github.com/sullo/nikto) (Web server vulnerability scanning)

Burp Suite (https://portswigger.net/burp) (Web proxy + testing suite)

John the Ripper (https://www.openwall.com/john/) (Password cracking (dictionary, brute force))

Hydra (https://www.kali.org/tools/hydra/) (Network login brute‑force testing)

Wireshark (https://www.wireshark.org/) (Packet capture + analysis)

TCPdump (https://www.redhat.com/en/blog/introduction-using-tcpdump-linux-command-line)
(Command‑line packet capture)

Aircrack-ng (https://www.aircrack-ng.org/) (Wi‑Fi security auditing)

Reaver WPS (https://www.kali.org/tools/reaver/) (WPS vulnerability testing)

Acunetix Web Application Security Scanner (https://www.acunetix.com/) (Web Vulnerability Scanner)

Cross Site Scripting (XSS)

## Hacking Tools (Used by AnonMafia Cyber Family)


1. [http://rootdown.cocolog-nifty.com/memo/2014/05/anonymous-opchi.html](http://rootdown.cocolog-nifty.com/memo/2014/05/anonymous-opchi.html)
2. [http://rootdown.cocolog-nifty.com/memo/2014/10/anonymous-ophon.html](http://rootdown.cocolog-nifty.com/memo/2014/10/anonymous-ophon.html)
3. https://miutony.blogspot.com/2014/10/analysis-of-cyber-warfare-weapons-on.html
4. https://sourceforge.net/projects/anonmafiacyberfamilydoser/
5. https://www.newsletter.co.uk/news/psni-75-of-major-cyber-crimeacross-ni-is-by-children-
    1097416
6. https://www.spywareremove.com/anonymous-hackers-plan-attacks-missouri-government-
    banking-sites.html
7. https://www.hipforums.com/threads/hacker-collective-anonymous-promises-to-take-down-
    missouri-government-and-bank-sites.464559/#post-
8. https://thecryptosphere.com/2014/11/17/opkkk-from-anonymous-defrocks-racists-across-
    america/
9. https://liberalamerica.org/2014/11/15/anonymous-kkk-ferguson/
10. https://techcrunch.com/2014/12/16/anonymous-allegedly-attacking-swedish-servers-for-
pirate-bay-shutdown/
11. https://x.com/anonmafiafamily
12. https://x.com/DerpLaughing
13. https://x.com/IncursioSubter
14. https://x.com/AnonRabb
15. https://anonmafiacyberfamily.wordpress.com/
16. https://www.linkedin.com/pulse/opnulldenmark-opbeast-darren-desmond
17. https://www.vice.com/en/article/teenage-hackers-promise-more-hacks-after-alleged-leaders-
arrest-cracka-cwa/
18. https://www.eurasiareview.com/24102015-hackers-who-cracked-cia-director-claim-they-
cant-track-us-down/
19. https://www.bleepingcomputer.com/news/security/hacker-incursio-gets-two-years-in-prison-
for-hacking-cia-dhs-doj-and-fbi/
20. https://www.rt.com/usa/319583-brennan-cia-hacked-wikileaks/
21. https://vimeo.com/
22. https://www.kali.org/tools/thc-ssl-dos/
23. https://kb.mazebolt.com/knowledgebase/tors-hammer-attack/
24. https://www.cloudflare.com/learning/ddos/ddos-attack-tools/slowloris/
25. https://www.cloudflare.com/learning/ddos/ddos-attack-tools/r-u-dead-yet-rudy/
26. https://medium.com/@dercraig/testing-github-ddos-tools-8-pyloris-d835239c83bf
27. https://www.radware.com/security/ddos-knowledge-center/ddospedia/hulk/
28. https://www.radware.com/security/ddos-knowledge-center/ddospedia/xerxes/
29. https://x.com/DerpLaughing/status/
30. https://www.a10networks.com/blog/doj-charges-six-for-ddos-for-hire-services-kills-48-sites/
31. https://www.justice.gov/usao-cdca/pr/federal-authorities-seize-13-internet-domains-
associated-booter-websites-offered-ddos
32. https://github.com/jjsantanna/booters_ecosystem_analysis/blob/master/booterblacklist.csv
33. https://x.com/KillingIAB
34. https://x.com/_BL4CK_FL4G_
35. https://x.com/RobertaLandrie/status/

## Sources (References)


36. https://x.com/xVen0mz
37. https://x.com/mrgumbys/status/
